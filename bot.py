...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if u.id == ADMIN_ID:
        if d == 'admin': await q.message.edit_text("👑 Админ", reply_markup=admin_main_kb()); return
            ...
            await q.message.edit_text("📅 Редактирование даты\n\nВведите ID пользователя:")
            return
...
async def reject_order(q, context, oid):
    ...
    await context.bot.send_message(o[1], f"❌ Заказ №{oid} отклонён. {ADMIN_USERNAME}")
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if key_id:
                ...
                if result:
                    new_exp, uid = result
                    sign = "+" if days >= 0 else ""
                    await context.bot.send_message(uid, f"📅 Ваша подписка изменена!\n\n🔑 Ключ ID: {key_id}\n⏱️ До: {format_date(new_exp)}\n📅 {sign}{days} дн.")
                await update.message.reply_text(f"✅ Дата изменена на {sign}{days} дн.\nДо: {format_date(new_exp) if result else '?'}", reply_markup=admin_main_kb())
            context.user_data.pop('edit_key_id', None); context.user_data.pop('edit_step', None)
        except: await update.message.reply_text("❌ Введите число (например: +7 или -3)"); return
    ...
    if context.user_data.get('wait_router') == 'name':
        ...
        rd = context.user_data['router_data']; name, phone, addr = rd['name'], rd['phone'], t
